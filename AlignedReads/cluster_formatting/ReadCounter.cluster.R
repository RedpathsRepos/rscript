...
# This script:  Takes a sam file with no header and counts all the reads 
...
input1 <- "contigs.txt"
out.filename <- paste(input,".processed", sep ="")


#==================================================================================================================#
#setwd(directory)
contigs <- read.table(input1, header=FALSE, sep="\t", na.strings="?", dec=".", strip.white=TRUE)
counts <- read.table(input[1], header=FALSE, sep="\t", na.strings="?", dec=".", strip.white=TRUE)  #could result in memory issues if very large
...
write.table(counted, out.filename, col.names = TRUE, row.names = FALSE, sep="\t", append = FALSE)  
